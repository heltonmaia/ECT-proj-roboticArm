...
def control_arm(arm: QSerialPort, COM: int, rotating_base_angle: int, gripper_angle: int, arm1_angle: int, arm2_angle: int):
    # Pack the data
    data = struct.pack('BBBB', rotating_base_angle, gripper_angle, arm1_angle, arm2_angle)
    
    # Send the data via serial port
    byte_array = QByteArray(data)
    arm.write(byte_array)
