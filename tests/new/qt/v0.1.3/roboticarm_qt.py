...
class thread_1(QThread):
    ...
    def run(self):
        ...
        self.arm = QSerialPort()
        self.arm.setPortName(f'COM{self.COM}')
        self.arm.setBaudRate(9600)
        self.arm.open(QSerialPort.OpenModeFlag.WriteOnly)

        if not self.arm.open(QSerialPort.OpenModeFlag.WriteOnly):
            print(f"Error while opening the port COM{self.COM}")
            ...
            if read:
                ...
                for box in boxes:
                    ...
                    if box.cls in [0, 1]:
                        ...
                        if in_range:
                            robotic_arm.control_arm(self.arm, self.COM, rotating_base_angle, gripper_angle, arm1_angle, arm2_angle)
