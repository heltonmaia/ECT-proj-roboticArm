...
from CommandClassifier.audio_classification import AudioClassificationThread
...
class RoboticArmAudioExecution(QWidget):
    def __init__(self):
        ...
        self.start_stop_button.clicked.connect(self.start_stop_audio_slot)

        self.audio_thread = AudioClassificationThread()
        self.audio_thread.audio_update_signal.connect(self.handle_audio_update)

    def start_stop_audio_slot(self):
    ...
    def handle_audio_update(self, command, confidence):
        print(f"Command: {command}, Confidence: {confidence}")
        # Aqui você pode adicionar lógica para controlar o braço robótico com base no comando
            ...
            if read:
                ...
                for box in boxes:
                    ...
                    if box.cls in [0, 1]:
                        ...
                        direction = detection_infos.calculate_direction(pixel_threshold, m_coord_x, m_coord_y,
                                                                        prev_m_coord_x, prev_m_coord_y)
                        ...
                        if in_range:
                            self.arm.write(
                                struct.pack('BBBB', rotating_base_angle, gripper_angle, arm1_angle, arm2_angle))
                ...
                try:
                    self.info_update_signal.emit(self.cap_device, self.COM, detected, in_range, hand_position, state,
                                                 direction, fps)
                except Exception as e:
                    print(e)
