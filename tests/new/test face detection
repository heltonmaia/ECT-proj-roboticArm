...
with mp_holistic.Holistic(min_detection_confidence=0.5,
    ...
    with mp_face_detection.FaceDetection(model_selection=1,
        ...
        while cap.isOpened():
            ...
            # Leitura e processamento de cada frame
            ...
            if results.right_hand_landmarks and face_results.detections:
                
                # Loop através das detecções de rosto
                for
                ...
                coord_landmarks_z = [landmark.z for landmark in landmarks_array]
                ...
                m_coord_z = np.mean(coord_landmarks_z)

                # Posição do ponto médio
                cv2.putText(image, f"({m_coord_x}, {m_coord_y})", (70, 50), 2, 0.35, (0, 0, 0), 1)

                # nose = results.face_landmarks.landmark[0]
                # nose_x = int(width * nose.x)
                # nose_y = int(height * nose.y)
                ...
                if (m_coord_x >= range_left and m_coord_x <= range_right) and (
                    ...
                    # Ponto do nariz
                    # cv2.circle(image, (nose_x, nose_y), 3, (0, 0, 255), 2)
